...
- A failing source is recorded on the CycleRun and does not fail the cycle.
  A per-document extract transport failure is recorded the same way and
  leaves that Document at `COLLECTED`.
- Repositories flush; they do not commit. A unit of work owns the session and
  ...
  work). Isolation around `extract()` covers transport failure before
  writes; persistence failure after a successful `extract()` still fails
  the stage.
