...
| `extraction` | Parse raw model output and judge it with deterministic rules into `accepted` / `proposed` / `rejected` (ADR-0002) | Implemented (S002-T007). Does not construct an `Event`. Duplicate fact/claim text is not a merge (ADR-0008). Cycle wiring and persistence are T008 | `domain`, `llm.artifacts` | SQLAlchemy, httpx, Anthropic SDK (`tests/unit/test_extraction_boundary.py`); `cycle`, `api`, `persistence` |
