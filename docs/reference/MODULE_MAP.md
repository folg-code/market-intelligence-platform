...
Last updated: 2026-08-19. Matches `sprint/first-llm-slice` after S002-T002..T008.
...
src/moj_projekt/
  ...
  extraction/    parser.py (raw text -> JSON object or parse failure),
                 ...
                 database. Cycle extract wiring is T009.
...
Dependency direction is one-way: `api` -> `cycle` -> `ingestion`/`persistence`/`llm` -> `domain`/`config`. `extraction` sits beside `llm` (`extraction` -> `domain` + `llm`; persistence via `UnitOfWork`; not yet called by `cycle`). Nothing imports upward.
