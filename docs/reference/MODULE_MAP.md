...
Last updated: 2026-08-18. Sprint 001 tasks S001-T002..T006 are merged; the
...
src/moj_projekt/
  ...
  domain/        document.py, source.py, repositories.py, enums.py, evidence.py,
                 embedding.py - value objects, invariants, repository interfaces
                 (implemented for Source/Document; other MVP entities land in
                 T007-T009)
  persistence/   models.py, document_repository.py, source_repository.py,
                 health.py - SQLAlchemy models, repository implementations,
                 the /health db+pgvector check (implemented for Source/Document)
...
migrations/      Alembic environment and versions (0001: enable pgvector
                 ...
                 immutability trigger on Document)
...
| `domain` | The model of `DOMAIN_MODEL.md`: value objects, invariants, repository interfaces | Implemented for Source, Document, enums, evidence, embedding descriptor; Event/EvidencePack/Narrative/impact/alert/governance types are T007-T009 | `config` (only for pure values) | SQLAlchemy, httpx, the Anthropic SDK - enforced by `tests/unit/test_domain_boundary.py` |
| `persistence` | Map domain objects to PostgreSQL; implement repository interfaces | Implemented for Source, Document (with DB-level immutability trigger + dedupe via unique constraint/`ON CONFLICT DO NOTHING`), plus the health check | `domain`, `config` | `ingestion`, `cycle`, `api` |
