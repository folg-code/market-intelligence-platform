...
def make_ingest_stage(*, adapters: Mapping[str, SourceAdapter]) -> Stage:
    """Build the ingest ``Stage`` bound to adapters.
    ...
    is registered in the same map; this loop does not change. Repositories
    come from the unit of work the cycle opens around this stage.
    """

    def run(cycle_run: CycleRun, uow: UnitOfWork) -> CycleRun:
        for source in uow.sources.list_active():
